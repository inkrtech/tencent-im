...
type (
	...
	createGroupReq struct {
		...
		InviteJoinOption string            `json:"InviteJoinOption,omitempty"` // （选填）邀请加群处理方式。包含 FreeAccess（自由加入），NeedPermission（需要验证），DisableApply（禁止邀请）不填默认为 DisableApply
	...
	GroupMsgGetSimpleRet struct {
		IsFinished int    // 是否返回了请求区间的全部消息 当成功返回了请求区间的全部消息时，值为1; 当消息长度太长或者区间太大（超过20）导致无法返回全部消息时，值为0; 当消息长度太长或者区间太大（超过20）且所有消息都过期时，值为2
		HasMore    bool   // 是否还有更多数据
		NextSeq    int    // 下一个消息Seq
		MsgSeqList []*int // 消息seq列表
	}
	...
	rspMsgItem struct {
		...
		MsgBody      interface{} `json:"MsgBody"`     //直播群IsSystemMsg = 0 和IsSystemMsg= 1 时，结构不一样
	...
	getMembersReq struct {
		GroupId   string `json:"GroupId"`   //（必填）操作的群ID
		Timestamp int    `json:"Timestamp"` //（必填）首次请求传0。当直播群中人数小于250人时，后台返回所有成员，并且返回 NextTimestamp 为0；当直播群中人数大于250人时，后台返回非0的 NextTimestamp，将该数字作为下一次请求的 Timestamp 拉取剩余人数。
		Mark      int    `json:"Mark"`      //（选填）按某个标记过滤拉取用户列表。
	}
